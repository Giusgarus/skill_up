...
def create_indexes(db: Optional[Database]) -> None:
    ...
    db["users"].create_index([("user_id", ASCENDING)], unique=True, name="ux_users_email")
    db["tasks"].create_index([("user_id", ASCENDING), ("task_id", ASCENDING)], unique=True, name="ux_tasks_user_date")
